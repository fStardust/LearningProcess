...
def retrieve_subcategory(request):
    if request.user.is_authenticated:
        ...
        # 使用 .first() 方法来安全地获取第一个对象或 None
        current_category = Category.objects.filter(name=category_type).first()

        if current_category is not None:
            subcategories = SubCategory.objects.filter(parent=current_category)
            subcategory_list = [(sc.id, sc.name) for sc in subcategories]
            return JsonResponse({"subcategories": subcategory_list})
        else:
            return JsonResponse({"subcategories": [(0, '重新选择大类')]})
